...
class ActsAsCommentableTest < Test::Unit::TestCase
  ...
  def test_create_comment
    ...
    assert_not_nil post.comments.create(:comment => "This is the a comment.").id

    wall = Wall.create(:name => "My Wall")
    assert_not_nil wall.public_comments.create(:comment => "This is the a comment.").id
    assert_not_nil wall.private_comments.create(:comment => "This is the a comment.").id
    assert_raise NoMethodError do
      wall.comments.create(:comment => "Title")
    end
  end

  def test_fetch_comments
    post = Post.create(:text => "Awesome post !")
    post.comments.create(:comment => "This is the first comment.")
    ...
    private_comment = wall.private_comments.create(:comment => "Yipiyayeah !")
    assert_equal [private_comment], wall.private_comments
    public_comment = wall.public_comments.create(:comment => "Yipiyayeah !")
  ...
  def test_find_comments_by_user
    ...
    post = Post.create(:text => "Awesome post !")
    comment = post.comments.create(:comment => "This is the first comment.", :user => user)
  ...
  def test_find_comments_for_commentable
    post = Post.create(:text => "Awesome post !")
    comment = post.comments.create(:comment => "This is the first comment.")
  ...
  def test_find_public_private_comments_by_user
    ...
    wall = Wall.create(:name => "wall")
    private_comment = wall.private_comments.create(:comment => "Yipiyayeah !", :user => user)
    assert_equal [private_comment], Wall.find_private_comments_by_user(user)

    public_comment = wall.public_comments.create(:comment => "Yipiyayeah !", :user => user)
  ...
  def test_comments_public_private_ordered_by_submitted
    post = Post.create(:text => "Awesome post !")
    comment = post.comments.create(:comment => "This is the first comment.")
    comment2 = post.comments.create(:comment => "This is the second comment.")
    assert_equal [comment, comment2], post.comments_ordered_by_submitted

    wall = Wall.create(:name => "wall")
    private_comment = wall.private_comments.create(:comment => "Yipiyayeah !")
    private_comment2 = wall.private_comments.create(:comment => "Yipiyayeah !")
    assert_equal [private_comment, private_comment2], wall.private_comments_ordered_by_submitted

    public_comment = wall.public_comments.create(:comment => "Yipiyayeah !")
    public_comment2 = wall.public_comments.create(:comment => "Yipiyayeah !")
  ...
  def test_add_comment
    post = Post.create(:text => "Awesome post !")
    comment = Comment.new(:comment => 'Super comment')
    post.add_comment(comment)
    assert_equal [comment], post.comments

    wall = Wall.create(:name => "wall")
    private_comment = Comment.new(:comment => 'Super comment')
    wall.add_private_comment(private_comment)
    assert_equal [private_comment], wall.private_comments

    public_comment = Comment.new(:comment => 'Super comment')
